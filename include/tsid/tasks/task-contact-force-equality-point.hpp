...
#ifndef __invdyn_task_contact_force_equality_point_hpp__
#define __invdyn_task_contact_force_equality_point_hpp__
...
class TaskContactForceEqualityPoint : public TaskContactForce {
 public:
  ...
  TaskContactForceEqualityPoint(const std::string& name, RobotWrapper& robot,
                           const double dt, contacts::ContactPoint& contact);

  virtual ~TaskContactForceEqualityPoint() {}

  int dim() const;

  virtual const std::string& getAssociatedContactName();
  virtual const contacts::ContactPoint& getAssociatedContact();
  void setAssociatedContact(contacts::ContactPoint& contact);
 ...
 protected:
  ...
  contacts::ContactPoint* m_contact;
  std::string m_contact_name;  // the associated contact name or an empty string
  ConstraintEquality m_constraint;
  TrajectorySample m_ref;   // reference Force 3D to follow
  TrajectorySample m_fext;  // external Force 3D in the same frame than the ref
